...
laspostprocessing <- function(las_file, spectral_file, boundary, crs_projection = NULL, las_out = NULL) {
  ...
  if (sum(str_detect(spectral_file, 'red(?!e)|green|blue|rededge|nir')) != 5) {
    stop('Incorrect spectral file band name. See documentation for details.',
         call. = FALSE)
  }
  ...
  spectral <- stack(
    raster_to_16bit(spectral_file, 'red(?!e)'),
  ...
  spectral$ndvi <-
    (spectral$nir - spectral$r) / (spectral$nir + spectral$r)
  ...
  ctgmergespatial = function(cluster) {
    ...
    las <- las %>%
      lasfilterduplicates() %>%
      lasmergespatial(stack(spectral$r,
                            spectral$g,
                            spectral$b)) %>%
      lasmergespatial(spectral$re,
                      attribute = 'RE') %>%
      lasmergespatial(spectral$nir,
                      attribute = 'N') %>%
      lasmergespatial(spectral$ndvi,
                      attribute = 'NDVI') %>%
    ...
    las <- las %>%
      lasfilterduplicates() %>%
      lasfilter(!(
        is.na(R) | is.na(G) | is.na(B) | is.na(RE) | is.na(N) | is.na(NDVI)
      ))
  ...
  options(future.globals.maxSize = 6291456000) # Increase to accomodate size of spectral stack
